...
def text_catcher(func, *args):
    txt_transform = f'{func.__name__.replace("_", " ").title()} [{", ".join(args)}]'
    print(txt_transform)
    return txt_transform

def open_browser(browser_name):
    actual_result = text_catcher(open_browser, browser_name)
    assert actual_result == "Open Browser [Chrome]"

def go_to_companyname_homepage(page_url):
    actual_result = text_catcher(go_to_companyname_homepage, page_url)
    assert actual_result == "Go To Companyname Homepage [https://companyname.com]"

def find_registration_button_on_login_page(page_url, button_text):
    actual_result = text_catcher(find_registration_button_on_login_page, page_url, button_text)
